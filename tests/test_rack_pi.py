...
class RackPiArchitectureTests(unittest.TestCase):
    ...
    def test_eaton_ups_has_ordered_thresholds_and_recovery(self) -> None:
        ...
        bootstrap = self.read("rack-pi/scripts/bootstrap.sh")
        journald = self.read("rack-pi/config/journald/90-rack-pi.conf")
        ...
        self.assertIn("UPS_CRITICAL_CONFIRM_SECONDS=30", ups_env)
        self.assertIn('driver = usbhid-ups', setup)
        self.assertIn('vendorid = 0463', setup)
        self.assertIn('ignorelb', setup)
        self.assertIn('override.battery.charge.low = ${UPS_RACK_SHUTDOWN_PERCENT}', setup)
        self.assertIn('override.battery.runtime.low = -1', setup)
        self.assertIn('upsmon -c fsd', orchestrator)
        self.assertIn('UPS critical candidate', orchestrator)
        self.assertIn("Storage=persistent", journald)
        self.assertIn("90-rack-pi.conf", bootstrap)
