#!/usr/bin/env python3
"""Private Telegram long-polling bridge for the n8n AI Ops workflow."""
...
def _request_json(
    ...
    with urllib.request.urlopen(request, timeout=timeout) as response:
        value = json.loads(response.read(MAX_BODY + 1))
...
def _poll() -> None:
    ...
    while True:
        try:
            ...
            response = _telegram("getUpdates", payload, timeout=40)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if self.path == "/v1/messages":
                ...
                if not isinstance(text, str) or not (1 <= len(text) <= 60_000):
